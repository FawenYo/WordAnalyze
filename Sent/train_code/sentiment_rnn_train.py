...
def train_model(model,train_x, train_y, test_x, test_y, val_x, val_y, batch_size, path) :
    ...
    saveBestModel = keras.callbacks.ModelCheckpoint(path+'/model/best_model1.hdf5', monitor='val_acc', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
def main():
    ...
    if train_flag:
        ...
        # load the dataset
        path = 'C:/Users/MyStyle/Desktop/WordAnalyze/Sent'
        data_dir = path+'/Data'
        ...
        model.save_weights(path+"/model/best_model_1.h5")
